...
const URGENCY_CONFIG: Record<string, { label: string; badge: string }> = {
  Routine: { label: 'Routine checkup', badge: 'badge-green' },
  Soon:    { label: 'See dentist soon', badge: 'badge-yellow' },
  Urgent:  { label: 'Urgent - see dentist now', badge: 'badge-red' },
};
        ...
        {findings.findings.map((f, i) => {
          ...
          return (
            <div key={i} className={`${styles.findingRow} ${f.severity === 'High' ? styles.findingHigh : f.severity === 'Medium' ? styles.findingMed : ''}`}>
              <div className={styles.findingIcon}>{CONDITION_CODES[f.condition] || 'DX'}</div>
